...
BUCKET_NAME = 'coustomerloyalty' # replace with your bucket name

s3 = boto3.resource('s3')

for key in ['train.csv', 'test.csv', 'historical_transactions.csv', 'merchants.csv', 'new_merchant_transactions.csv']:
    target = os.path.join(arg.datapath, key)
...
accuracydata = accuracy.find({})
accuracydatalist = []
for item in accuracydata:
    accuracydatalist.append(item['accuracy'])
old_accuracy = accuracydatalist[-1]
#print(accuracy)
...
def train_test(num_rows=None):
    ...
    train_df = pd.read_csv(
        os.path.join(arg.datapath, 'train.csv'),
        index_col=['card_id'],
        nrows=num_rows)
    test_df = pd.read_csv(
        os.path.join(arg.datapath, 'test.csv'),
        index_col=['card_id'],
        nrows=num_rows)
...
def historical_transactions(num_rows=None):
    # load csv
    hist_df = pd.read_csv(
        os.path.join(arg.datapath, 'historical_transactions.csv'),
        nrows=num_rows)
...
def new_merchant_transactions(num_rows=None):
    ...
    new_merchant_df = pd.read_csv(
        os.path.join(arg.datapath, 'new_merchant_transactions.csv'),
        nrows=num_rows)
...
def kfold_lightgbm(train_df, test_df, num_folds, stratified = False, debug= False):
    ...
    for n_fold, (train_idx, valid_idx) in enumerate(folds.split(train_df[feats], train_df['outliers'])):
        ...
        predict_valid_accuracy = rmse(valid_y, oof_preds[valid_idx])
        print('Fold %2d RMSE : %.6f' % (n_fold + 1, predict_valid_accuracy))
        predictresults.append(predict_valid_accuracy)
        # insert accuracy to database
...
def main(debug=False):
    ...
    with timer("Run LightGBM with kfold"):
        predictresults= kfold_lightgbm(train_df, test_df, num_folds=2, stratified=False, debug=debug)
